...
const insertIntoDB = async (
  ...
  if (isAnySemesterUpcomingOrOngoing) {
    throw new ApiError(
      ...
      ` There is already an ${isAnySemesterUpcomingOrOngoing.status} semester registration`
    );
  }
...
const startMyRegistration = async (
  ...
  if (semesterRegistrationInfo?.status === SemesterRegistrationStatus.UPCOMING)
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      'semester registration is started yet'
    );
...
const confirmMyRegistration = async (
  ...
  if (studentSemesterRegistration.totalCreditsTaken === 0)
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      'Your are not enrolled in any course'
    );
...
const startNewSemester = async (id: string): Promise<{ message: string }> => {
  ...
  await prisma.$transaction(async clientTransaction => {
    ...
    const studentSemesterRegistrations =
      await clientTransaction.studentSemesterRegistration.findMany({
        where: {
          semesterRegistration: {
            id,
          },
    ...
    asyncForEach(
      ...
      async (studentSemReg: StudentSemesterRegistration) => {
        ...
        const studentSemesterRegistrationCourses =
          await clientTransaction.studentSemesterRegistrationCourse.findMany({
            where: {
              semesterRegistration: {
                id,
              },
        ...
        asyncForEach(
          ...
          async (
            ...
            const isExistEnrolledData =
              await clientTransaction.studentEnrolledCourse.findFirst({
                where: {
            ...
            if (!isExistEnrolledData) {
              ...
              const studentEnrolledCourseData =
                await clientTransaction.studentEnrolledCourse.create({
...
const getMySemesterRegCourses = async (authUserId: any) => {
