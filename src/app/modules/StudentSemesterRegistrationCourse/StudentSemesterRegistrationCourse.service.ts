...
const enrollIntoCourse = async(authUserId: string, payload: IEnrollCoursePayload): Promise<{message: string}> => {
    ...
    if(!semesterRegistration) throw new ApiError(httpStatus.NOT_FOUND, "Semester Registration not found")
    if(!offeredCourse) throw new ApiError(httpStatus.NOT_FOUND, "Offered Course not found")
    if(!offeredCourseSection) throw new ApiError(httpStatus.NOT_FOUND, "Offered Course Section not found")
    if(offeredCourseSection.maxCapacity && offeredCourseSection.currentlyEnrolledStudent && 
